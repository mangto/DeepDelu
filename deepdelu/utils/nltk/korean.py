...
def get_jamo_type(c:str):
    check_hangul(c)
    assert is_hangul_compat_jamo(c), f"not a jamo: {ord(c):x}"
    return sum(t for t, s in CHAR_SETS.items() if c in s)


def split_syllable_char(c:str) -> tuple:
    check_hangul(c)
    if len(c) != 1:
...
def split_syllables(s:str, ignore_err=True, pad=None) -> str:
    '''
    안녕하세요 -> ㅇㅏㄴㄴㅕㅇㅎㅏㅅㅔㅇㅛ
    '''
    def try_split(c):
...
def join_jamos(s:str, ignore_err=True) -> str:
    '''
    ㅇㅏㄴㄴㅕㅇㅎㅏㅅㅔㅇㅛ -> 안녕하세요
    '''
    last_t = 0
